...
func CheckToPauseContainer(container global.ContainerData, pauseContainerList []global.PauseContainer) bool {
	for _, pauseContainer := range pauseContainerList {
		if pauseContainer.ContainerName == container.Name {
			return false
		}
	}
	return true
...
func ContinueContainer(client internalapi.RuntimeService, continueCandicate *global.ContainerData) {
	continueCandicate.OriginalContainerData.Linux.CpuQuota = 0
